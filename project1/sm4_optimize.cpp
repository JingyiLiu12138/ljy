...
int main()
{
    ...
    string input;
    input = "jingyiLiu"; 
    size_t orglen = input.size(); 
    size_t padlen = 16 - (orglen % 16); 
    vector<uint8_t> buffer(orglen + padlen);
    memcpy(buffer.data(), input.data(), orglen);
    memset(buffer.data() + orglen, (uint8_t)padlen, padlen);
    //CBC模式加密
    vector<uint8_t> cipher(buffer.size());
    uint8_t IV_pre[16];
    memcpy(IV_pre, IV, 16); 
    LARGE_INTEGER Freq; //API计时器
    LARGE_INTEGER Start;
    LARGE_INTEGER End;
    ...
    for (size_t offset = 0; offset < buffer.size(); offset += 16) {
        ...
        encrypt(block, &cipher[offset], rk); 
        memcpy(IV_pre, &cipher[offset], 16); 
    }
